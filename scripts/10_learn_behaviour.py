...
def truth(segment: str) -> dict:
    """Hidden buyer response for a segment; the resolver never sees this."""
    ch, tier, band = segment.split("|")
    t = int(tier[1:])
    ds = min(0.6, max(0.02, ECONOMICS.stepup_abandon_rate * CHANNEL_FACTOR[ch] + TIER_ADD[t] + BAND_ADD[band]))
    db = 0.65 - (0.07 if ch == "SOCIAL" else 0.0) + (0.06 if t >= 3 else 0.0)
    rho = 0.10 + (0.05 if t >= 3 else 0.0)
    dp = min(0.8, max(0.1, ECONOMICS.prepaid_abandon_rate * CHANNEL_FACTOR[ch] + TIER_ADD[t]))
...
def main() -> None:
    ...
    print(f"[stream] {len(st):,} COD orders from the conformal + test eras | learner prior delta_s {ECONOMICS.stepup_abandon_rate:.0%}")

    for i, row in enumerate(st.itertuples(index=False)):
        ts = float(row.ts)
        while pending and pending[0][0] <= ts:                       # delayed outcomes arrive
            _, kind, seg_, p_, ab, rt = heapq.heappop(pending)
            (learner.observe_stepup(seg_, p_, ab, rt) if kind == "stepup" else learner.observe_prepaid(seg_, p_, ab))
        ...
        if a == STEP_UP:
            ...
            rho_eff = tr["rho"] + (1 - tr["rho"]) * ctx_prior.address_attribution
            rto = None if abandoned else (bad and rng.random() < rho_eff)
            heapq.heappush(pending, (ts + DELAY, "stepup", seg, p, abandoned, rto))
        elif a == PREPAID:
            abandoned = rng.random() < (0.85 if bad else tr["delta_p"])
            heapq.heappush(pending, (ts + DELAY, "prepaid", seg, p, abandoned, None))
        if (i + 1) % 250 == 0:
            top = sorted(seg_count, key=seg_count.get, reverse=True)[:4]
            trajectory.append({"orders": i + 1, "pnl": dict(pnl), "estimates": {
                s: {"delta_s": learner.estimate(s).delta_s, "truth": truth(s)["delta_s"], "source": learner.estimate(s).source} for s in top}})

    rows = []
    for s, n in sorted(seg_count.items(), key=lambda kv: -kv[1]):
        e, t = learner.estimate(s), truth(s)
    ...
    summary = {
        "orders": int(len(st)), "pnl": pnl, "actions": actions,
    ...
    print(f"[pnl] prior {pnl['prior']:,.0f} | learned {pnl['learned']:,.0f} | oracle {pnl['oracle']:,.0f} | recovered "
          f"{summary['recovered_share'] if summary['recovered_share'] is None else f'{summary['recovered_share']:.0%}'} of the gap")
    print(f"[fit] {summary['segments_learned']} segments learned from {learner.observations:,} observations | mean |delta_s error| "
          f"prior {summary['mean_abs_err_delta_s_prior']} -> learned {summary['mean_abs_err_delta_s_learned']}")
    for r in rows[:6]:
    ...
    (REPORT_DIR / "behaviour.json").write_text(json.dumps({"summary": summary, "segments": rows, "trajectory": trajectory,
                                                            "truth_model": "delta_s = prior x channel factor + tier + basket band"}, indent=2), encoding="utf-8")
    learner.snapshot(BEHAVIOUR_PATH)
